...
function makePayslipPDF(emp,d,m,y,orgName,orgEmail,orgPos,logoSrc){
  loadJsPDFGlobal(function(JsPDF){
    ...
    downloadPDF(doc.output("blob"),"Payslip-"+(emp.name||"").replace(/ /g,"-")+"-"+MOS[m]+"-"+y+".pdf");
...
function makePayrollPDF(emps,m,y,mAttFn,incFn,orgName,orgEmail,orgPos,logoSrc){
  loadJsPDFGlobal(function(JsPDF){
    ...
    var cols=["EMPLOYEE NAME","DEPARTMENT","GROSS","DEDUCTIONS","NET PAY","TOTAL CTC"];
    var cws=[62,45,35,35,35,37];
    ...
    emps.forEach(function(emp,ei){
      ...
      doc.setTextColor(180,110,0);doc.setFont("helvetica","normal");doc.text(fmtIN(d.gr+d.pfR+d.esiR),cx[5]+cws[5]-3,ry+6,{align:"right"});
      ry+=rh;
    ...
    doc.text("Gross: "+fmtIN(tG)+"   |   Deductions: "+fmtIN(tD)+"   |   Total CTC: "+fmtIN(tG+tP+tE),W-mg-5,ry+11,{align:"right"});
    pdfFooter(doc,W,mg,H,orgName,orgEmail);
