...
"location_off":"<path d=\"M12 21s-7-6.5-9-11a9 9 0 0 1 1.72-5.28\"/><path d=\"M14.9 4.24A9 9 0 0 1 21 10c0 4-3 8-6.4 10.5\"/><path d=\"M9 9a3 3 0 0 0 4.24 4.24\"/><line x1=\"2\" x2=\"22\" y1=\"2\" y2=\"22\"/>",
"my_location":"<circle cx=\"12\" cy=\"12\" r=\"3\"/><path d=\"M12 2v4\"/><path d=\"M12 18v4\"/><path d=\"M2 12h4\"/><path d=\"M18 12h4\"/><circle cx=\"12\" cy=\"12\" r=\"8\"/>",
...
// Distance in meters between two lat/lng points — used to verify an office-mode employee is
// actually within range of their workplace when they check in.
...
export default function App(){
  ...
  // Self check-in summary (total working hours) for whichever employee the employer is currently
  // viewing — fetched here, at the top level, not inside the accordion's render callback, so this
  // hook always runs the same way on every render regardless of whether that section is expanded.
  var sSelfCheckinSummary=st(null),selfCheckinSummary=sSelfCheckinSummary[0],setSelfCheckinSummary=sSelfCheckinSummary[1];
  se(function(){
    if(!selE||!selE.appEmail){setSelfCheckinSummary(null);return;}
    var y=curY,m=curM;
    _sb.from("attendance_checkins").select("*").eq("employee_email",selE.appEmail)
      .gte("checkin_date",y+"-"+String(m+1).padStart(2,"0")+"-01")
      .lte("checkin_date",y+"-"+String(m+1).padStart(2,"0")+"-31")
    .then(function(res){
      if(!res.data){setSelfCheckinSummary(null);return;}
      var totalMin=0,days=0;
      res.data.forEach(function(c){
        if(c.check_in_at&&c.check_out_at){
          var mins=(new Date(c.check_out_at)-new Date(c.check_in_at))/60000;
          if(mins>0){totalMin+=mins;days++;}
        }
      });
      setSelfCheckinSummary({totalMin:totalMin,days:days,rows:res.data});
    });
  },[selE&&selE.id,selE&&selE.appEmail,curY,curM]);
  ...
  // Office GPS location — used to verify office-mode employees are physically on-site when they check in.
  var sOGeo=st(function(){var gu=lsGet("hr_guser",null);if(!gu||!gu.email)return {lat:null,lng:null,radius:200};var o=lsGet("hr_org_"+gu.email,{});return {lat:o.officeLat||null,lng:o.officeLng||null,radius:o.geoRadius||200};}),orgGeo=sOGeo[0],setOrgGeo=sOGeo[1];
  var sGeoCapturing=st(false),geoCapturing=sGeoCapturing[0],setGeoCapturing=sGeoCapturing[1];
      ...
      Promise.all([
        ...
        if(role==="employee"||role==="terminated_employee"){
          ...
          Promise.all([
            ...
            setOrg({
              ...
              officeLat:(empOrg&&empOrg.office_lat)||null,
              officeLng:(empOrg&&empOrg.office_lng)||null,
              geoRadius:(empOrg&&empOrg.geo_radius_m)||200
            });
        ...
        if(orgRes.data&&orgRes.data.org_name){
          var o={
            ...
            officeLat:(orgRes.data&&orgRes.data.office_lat)||null,
            officeLng:(orgRes.data&&orgRes.data.office_lng)||null,
            geoRadius:(orgRes.data&&orgRes.data.geo_radius_m)||200
          };
          lsSet("hr_org_"+em,o);setOrg(o);
          setOrgGeo({lat:o.officeLat,lng:o.officeLng,radius:o.geoRadius});
  ...
  // Periodically re-fetch leave requests + notifications while the app is open, for whoever is
  // currently logged in — without this, a request submitted by an employee only ever reached the
  // employer's screen on their NEXT full login, since loadLeaveAndNotifs previously only ran once
  // at session start. This is what actually makes "employer not receiving it" feel fixed, since the
  // data was always arriving correctly (verified directly against the database) — it just never
  // refreshed into view while someone was already using the app.
  se(function(){
    if(!gUser||!gUser.email)return;
    var isEmpSession3=userRole==="employee"||userRole==="terminated_employee";
    var pollEmail=gUser.email;
    var pollRole=isEmpSession3?"employee":userRole;
    var pollEmployerEmail=isEmpSession3?empEmployerEmail:null;
    if(isEmpSession3&&!pollEmployerEmail)return; // wait until employer link resolves
    var t=setInterval(function(){
      loadLeaveAndNotifs(pollEmail,pollRole,pollEmployerEmail);
    },25000);
  ...
  function padPin(pin){return pin.length===4?pin+pin:pin;}
  ...
  // Marks one notification read — updates local state AND the database. Previously "read" only
  // ever changed in local memory, so every reload or 25-second poll re-fetched the same
  // notification still marked unread from Supabase and put it right back, which looked exactly
  // like getting the same notification again and again.
  function markNotifRead(id){
    setNotifs(function(p){return p.map(function(x){return x.id===id?Object.assign({},x,{read:true}):x;});});
    setUnreadNotifs(function(c){return Math.max(0,c-1);});
    _sb.from("notifications").update({read:true}).eq("id",id).then(function(){});
  }
  function markAllNotifsRead(){
    var myEmail=gUser&&gUser.email;
    var unreadIds=notifs.filter(function(n){return n.to===myEmail&&!n.read;}).map(function(n){return n.id;});
    setNotifs(function(p){return p.map(function(n){return n.to===myEmail?Object.assign({},n,{read:true}):n;});});
    setUnreadNotifs(0);
    if(unreadIds.length)_sb.from("notifications").update({read:true}).in("id",unreadIds).then(function(){});
  }
  // Lets a person clear old/unwanted notifications from their own inbox — deletes from the
  // database too, not just the local list, so it doesn't reappear on the next reload/poll.
  function deleteNotif(id){
    setNotifs(function(p){return p.filter(function(x){return x.id!==id;});});
    _sb.from("notifications").delete().eq("id",id).then(function(){});
  }
  function deleteAllReadNotifs(){
    var myEmail=gUser&&gUser.email;
    var readIds=notifs.filter(function(n){return n.to===myEmail&&n.read;}).map(function(n){return n.id;});
    if(!readIds.length)return;
    setNotifs(function(p){return p.filter(function(n){return !(n.to===myEmail&&n.read);});});
  ...
  function addTaskComment(taskId){
    ...
    // Persist to the real task_comments table so the other side (employer or employee) actually
    // receives it — this previously only ever updated the sender's own local memory, so neither
    // side could see an "update message" the other one left, no matter how the UI displayed it.
    _sb.from("task_comments").insert({id:comment.id,task_id:String(taskId),from_email:comment.fromEmail,message:comment.message}).then(function(r){
      if(r&&r.error)showT("Comment could not be saved: "+r.error.message,"err");
    });
    var task=tasks.find(function(t){return t.id===taskId;});
    if(task){
      var toEmail=gUser.email===task.employerEmail?task.assignTarget:task.employerEmail;
  ...
  // Loads every comment for every task belonging to the current employer (whether you're the
  // owner or a linked employee) — matches the same employer-email resolution pattern used for
  // leave/loans/revisions/KPIs, since task_comments is keyed by task_id, not directly by employer.
  se(function(){
    if(!gUser||!gUser.email)return;
    var isEmpSession2=userRole==="employee"||userRole==="terminated_employee";
    var myTaskIds=(tasks||[]).filter(function(t){
      return isEmpSession2?(t.employerEmail===empEmployerEmail):(t.employerEmail===gUser.email);
    }).map(function(t){return String(t.id);});
    if(!myTaskIds.length)return;
    _sb.from("task_comments").select("*").in("task_id",myTaskIds).order("created_at",{ascending:true}).then(function(res){
      if(!res.data)return;
      var grouped={};
      res.data.forEach(function(c){
        var tid=isNaN(Number(c.task_id))?c.task_id:Number(c.task_id);
        grouped[tid]=(grouped[tid]||[]).concat([{id:c.id,taskId:tid,fromEmail:c.from_email,message:c.message,createdAt:c.created_at}]);
      });
      setTaskComments(grouped);
    });
  },[tasks.length,gUser&&gUser.email,userRole,empEmployerEmail]);
  ...
  var sLeaveSubmitting=st(false),leaveSubmitting=sLeaveSubmitting[0],setLeaveSubmitting=sLeaveSubmitting[1];
  function applyLeave(){
    ...
    // same table — actually receives it. The form now stays open with a visible "Sending..." state
    // until this genuinely succeeds or fails — previously the form closed and cleared immediately
    // on tap, before the request had actually reached the server, which is what made a failed
    // submission look like it "worked" with no way to tell.
  ...
  function approveLeave(reqId){
    setLeaveReqs(function(p){return p.map(function(r){
      ...
      setAtt(function(a){
        ...
        // work email — never guess by name, which could silently mark the wrong person's attendance.
        var emp=emps.find(function(e){return (e.appEmail&&e.appEmail===r.employeeEmail)||e.email===r.employeeEmail;});
        while(d<=end){
...
null
  ...
  function renderEmpDetail(){
    ...
    return h("div",{className:"fd"},
      ...
      selE.appLinked?accSection("selfcheckin","schedule",TEL,"Self Check-in",
        ...
        function(){
          if(!selfCheckinSummary)return h("div",{style:{fontSize:11,color:GRY,padding:"6px 0"}},"No self check-in records for "+MOS[curM]+" "+curY+".");
          var hrs=Math.floor(selfCheckinSummary.totalMin/60),mins=Math.round(selfCheckinSummary.totalMin%60);
          return h("div",{style:{display:"flex",flexDirection:"column",gap:8}},
            h("div",{style:{background:NVY,borderRadius:12,padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}},
              h("div",null,
                h("div",{style:{fontSize:9.5,color:"rgba(255,255,255,.5)"}},"Total working hours — "+MOS[curM]),
                h("div",{style:{fontSize:17,fontWeight:800,color:"#fff",marginTop:2}},hrs+"h "+mins+"m")
              ),
              h("div",{style:{textAlign:"right"}},
      ...
      (function(){
        ...
        if(!empLeaves.length)return null;
        return accSection("leaverequests","event_available",AMB,"Leave Requests",
      ...
      card(h("div",null,
        ...
        h("div",{style:{display:"flex",gap:8,marginBottom:10}},
          [["office","Office","location_on"],["remote","Remote / WFH","home"]].map(function(m){
            var on=(editE.workMode||"office")===m[0];
            return h("button",{key:m[0],type:"button",onClick:function(){setField("workMode",m[0]);},style:{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,background:on?ACCENT:SFT,border:"1.5px solid "+(on?ACCENT:BDR),borderRadius:10,padding:"10px",color:on?ACCENT_FG:NVY,fontSize:12,fontWeight:600,cursor:"pointer"}},ic(m[2],on?ACCENT_FG:NVY,14),m[1]);
          })
        ),
        h("div",{style:{fontSize:10,color:GRY,marginBottom:2,lineHeight:1.4}},(editE.workMode||"office")==="office"?"Must be near the office location (set in Business Settings) to check in.":"Can check in from anywhere — no location check.")
      )),
      card(h("div",null,
    ...
    var companyTab=settTab==="company"?h("div",null,
      card(h("div",null,
        ...
        !settReviewMode?h("div",null,
          // ── Editable: Office Location (GPS attendance) ──
          sectionTitle("OFFICE LOCATION — FOR GPS ATTENDANCE"),
          h("div",{style:{fontSize:11,color:GRY,lineHeight:1.5,marginBottom:10}},"Set your workplace location once so office-mode employees can only check in when they're actually on-site. Remote/WFH employees skip this check."),
          orgGeo.lat?h("div",{style:{background:GRN+"10",border:"1px solid "+GRN+"33",borderRadius:10,padding:"10px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:8}},
            ic("location_on",GRN,16),
            h("div",{style:{flex:1}},
              h("div",{style:{fontSize:11.5,fontWeight:700,color:GRN}},"Location Set"),
              h("div",{style:{fontSize:10,color:GRY,marginTop:1}},orgGeo.lat.toFixed(5)+", "+orgGeo.lng.toFixed(5)+" — "+orgGeo.radius+"m radius")
            )
          ):h("div",{style:{background:AMB+"10",border:"1px solid "+AMB+"33",borderRadius:10,padding:"10px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:8}},
            ic("location_off",AMB,16),
            h("div",{style:{fontSize:11.5,color:AMB,fontWeight:600}},"Not set yet — office check-in won't verify location until you set this")
          ),
          h("div",{style:{display:"flex",gap:8,marginBottom:10}},
            h("button",{onClick:function(){
              if(!navigator.geolocation)return showT("Location not supported on this device","err");
              setGeoCapturing(true);
              navigator.geolocation.getCurrentPosition(function(pos){
                setOrgGeo(function(p){return Object.assign({},p,{lat:pos.coords.latitude,lng:pos.coords.longitude});});
                setGeoCapturing(false);
                showT("Location captured! Remember to save.");
              },function(err){setGeoCapturing(false);showT("Could not get location: "+err.message,"err");},{enableHighAccuracy:true,timeout:10000});
            },style:{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,background:ACCENT+"12",border:"1.5px solid "+ACCENT+"30",borderRadius:10,padding:"11px",color:ACCENT,fontSize:12,fontWeight:700,cursor:"pointer"}},
              geoCapturing?"Getting location...":[ic("my_location",ACCENT,15),"Use My Current Location"]
            )
          ),
          orgGeo.lat?h("div",{style:{marginBottom:14}},
            lbl("CHECK-IN RADIUS (METERS)"),
            h("input",{type:"number",value:orgGeo.radius,onChange:function(e){setOrgGeo(function(p){return Object.assign({},p,{radius:Number(e.target.value)||200});});},style:{width:"100%",background:SFT,border:"1.5px solid "+BDR,borderRadius:10,padding:"10px 12px",fontSize:13,color:NVY,outline:"none",fontFamily:"inherit",boxSizing:"border-box"}})
          ):null,
          ...
          h("div",{style:{display:"flex",gap:8}},
            ...
            h("button",{onClick:function(){
              var updated=Object.assign({},org,{address:orgAddr,logo:orgLogo,phone:orgPhone,website:orgWebsite,contactEmail:orgContactEmail,officeLat:orgGeo.lat,officeLng:orgGeo.lng,geoRadius:orgGeo.radius});
              setOrg(updated);
              ...
              _sb.from("user_orgs").upsert({
                ...
                office_lat:orgGeo.lat,
                office_lng:orgGeo.lng,
                geo_radius_m:orgGeo.radius
  ...
  function renderEmployeeDashboard(){
    ...
    // ── GPS Check-in ──
    // Self check-in lives in its own dedicated table (attendance_checkins), NOT the payroll
    // att_json blob — an employee genuinely cannot be given write access to that (it's protected
    // for security reasons, and there's no safe way to restrict them to only their own entries
    ...
    function doCheckIn(){
      ...
      var workMode=myRecord.workMode||"office";
      setCheckinLoading(true);
      function saveCheckin(lat,lng,distanceM,withinRange){
        var row={id:Date.now(),employee_email:gUser.email,employer_email:empEmployerEmail||org.email,checkin_date:todayStr,check_in_at:new Date().toISOString(),check_in_lat:lat,check_in_lng:lng,distance_m:distanceM,work_mode:workMode,within_range:withinRange};
        _sb.from("attendance_checkins").upsert(row,{onConflict:"employee_email,checkin_date"}).then(function(res){
          setCheckinLoading(false);
          if(res.error){showT("Check-in failed: "+res.error.message,"err");return;}
          setCheckinToday(row);
          setCheckinMonth(function(p){return [row].concat(p.filter(function(r){return r.checkin_date!==todayStr;}));});
          showT(withinRange===false?"Checked in — outside office range, flagged for HR review":"Checked in!");
        });
      }
      if(workMode==="remote"){saveCheckin(null,null,null,null);return;}
      if(!navigator.geolocation){saveCheckin(null,null,null,null);return;}
      navigator.geolocation.getCurrentPosition(function(pos){
        var lat=pos.coords.latitude,lng=pos.coords.longitude;
        if(org.officeLat&&org.officeLng){
          var dist=geoDistanceM(lat,lng,org.officeLat,org.officeLng);
          var within=dist<=(org.geoRadius||200);
          if(!within){
            setCheckinLoading(false);
            showT("You're "+Math.round(dist)+"m from the office (limit "+(org.geoRadius||200)+"m). Move closer, or contact HR.","err");
            return;
          }
          saveCheckin(lat,lng,Math.round(dist),true);
        }else{
          saveCheckin(lat,lng,null,null); // office location not set by employer yet — allow through
        }
      },function(err){setCheckinLoading(false);showT("Could not get your location: "+err.message,"err");},{enableHighAccuracy:true,timeout:10000});
    }
    function doCheckOut(){
      if(!checkinToday)return;
      setCheckinLoading(true);
      _sb.from("attendance_checkins").update({check_out_at:new Date().toISOString()}).eq("id",checkinToday.id).then(function(res){
        setCheckinLoading(false);
        if(res.error){showT("Check-out failed: "+res.error.message,"err");return;}
        var upd=Object.assign({},checkinToday,{check_out_at:new Date().toISOString()});
        setCheckinToday(upd);
        setCheckinMonth(function(p){return p.map(function(r){return r.id===upd.id?upd:r;});});
        showT("Checked out. Have a good day!");
      });
    }
    ...
    function empHome(){
      var now=new Date();
      var emp2=myRecord;
      var isCheckedIn=!!(checkinToday&&checkinToday.check_in_at);
      var isCheckedOut=!!(checkinToday&&checkinToday.check_out_at);
      var workMode=(emp2&&emp2.workMode)||"office";
      var checkInTime=isCheckedIn?new Date(checkinToday.check_in_at).toLocaleTimeString("en-IN",{hour:"numeric",minute:"2-digit"}):null;
      var checkOutTime=isCheckedOut?new Date(checkinToday.check_out_at).toLocaleTimeString("en-IN",{hour:"numeric",minute:"2-digit"}):null;
      // Soft neumorphic shadow pair — light source from the top-left, using genuine dark/light tints
      // instead of a random gray-blue, so it reads correctly in both this app's light and dark mode.
      var neuLight=themeMode==="light"?"6px 6px 12px rgba(0,0,0,.09),-6px -6px 12px rgba(255,255,255,.85)":"6px 6px 12px rgba(0,0,0,.45),-4px -4px 10px rgba(255,255,255,.03)";
      var neuInset=themeMode==="light"?"inset 4px 4px 8px rgba(0,0,0,.08),inset -4px -4px 8px rgba(255,255,255,.8)":"inset 4px 4px 8px rgba(0,0,0,.4),inset -3px -3px 7px rgba(255,255,255,.03)";
      var allRows=[
        emp2&&emp2.mob?["Mobile",emp2.mob]:null,
        emp2&&emp2.email?["Email",emp2.email]:null,
        emp2&&emp2.joined?["Joined",emp2.joined]:null,
        emp2&&emp2.pan?["PAN",emp2.pan.slice(0,3)+"XXXXX"+emp2.pan.slice(-2)]:null,
        emp2&&emp2.uan?["UAN",emp2.uan.slice(0,3)+"XXXXX"+emp2.uan.slice(-2)]:null,
        emp2&&emp2.aadhar?["Aadhaar","XXXX XXXX "+emp2.aadhar.slice(-4)]:null,
        ["Employer",org.position||"—"],
        org.phone?["Company Phone",org.phone]:null,
      ...
      return h("div",null,
        ...
        h("div",{style:{background:SFT,borderRadius:18,padding:"14px",marginBottom:12,boxShadow:neuLight}},
          h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}},
            h("div",{style:{fontSize:12,fontWeight:700,color:NVY,display:"flex",alignItems:"center",gap:6}},ic("insights",AMB,15),"Performance — "+MOS[mo]),
            h("div",{style:{fontSize:12,fontWeight:700,color:attPct>=90?GRN:attPct>=70?ACCENT:RED}},attPct+"%")
          ),
          h("div",{style:{height:6,background:BDR,borderRadius:3,overflow:"hidden",marginBottom:8}},
            h("div",{style:{height:"100%",background:attPct>=90?GRN:attPct>=70?ACCENT:RED,borderRadius:3,width:attPct+"%",transition:"width 0.8s"}})
          ),
          h("div",{style:{display:"flex",justifyContent:"space-between",fontSize:10.5,color:GRY}},
            h("span",null,"Attendance "+attPct+"%"),
            h("span",null,completedThisMonth+" tasks done this month")
          )
        ),
        // Notifications
        unread>0?h("div",{style:{background:SFT,borderRadius:18,padding:"14px",marginBottom:12,boxShadow:neuLight}},
          h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}},
          ...
          myNotifs.slice(0,3).map(function(n){
            return h("div",{key:n.id,onClick:function(){markNotifRead(n.id);},style:{display:"flex",gap:8,padding:"6px 0",borderBottom:"1px solid "+BDR,cursor:"pointer",alignItems:"flex-start"}},
    ...
    function empAttendance(){
      ...
      // Total working hours this month — computed from real check-in/out timestamps, not the
      // HR-marked status. Only counts days with both a check-in AND check-out recorded.
      var totalMinutes=0,daysWithHours=0;
      checkinMonth.forEach(function(c){
        if(c.check_in_at&&c.check_out_at){
          var mins=(new Date(c.check_out_at)-new Date(c.check_in_at))/60000;
          if(mins>0){totalMinutes+=mins;daysWithHours++;}
        }
      });
      var totalHrs=Math.floor(totalMinutes/60),totalMins=Math.round(totalMinutes%60);
      var checkinByDate={};
      checkinMonth.forEach(function(c){checkinByDate[c.checkin_date]=c;});
      ...
      return h("div",null,
        ...
        // ── Total working hours — computed from real check-in/out timestamps ──
        h("div",{style:{background:NVY,borderRadius:14,padding:"14px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}},
          h("div",null,
            h("div",{style:{fontSize:10,color:"rgba(255,255,255,.5)"}},"Total working hours"),
            h("div",{style:{fontSize:20,fontWeight:800,color:"#fff",marginTop:2}},totalHrs+"h "+totalMins+"m")
          ),
          h("div",{style:{textAlign:"right"}},
            h("div",{style:{fontSize:10,color:"rgba(255,255,255,.5)"}},"Days tracked"),
            h("div",{style:{fontSize:16,fontWeight:700,color:"#fff",marginTop:2}},daysWithHours)
          )
        ),
        h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}},
          h("div",{style:{fontSize:10.5,color:GRY,display:"flex",alignItems:"center",gap:5}},ic("verified",GRY,12),"Official record — confirmed by HR"),
          h("button",{onClick:function(){setShowAttTimes(!showAttTimes);},style:{display:"flex",alignItems:"center",gap:4,background:showAttTimes?ACCENT+"12":SFT,border:"1px solid "+(showAttTimes?ACCENT+"30":BDR),borderRadius:8,padding:"4px 9px",color:showAttTimes?ACCENT:GRY,fontSize:10,fontWeight:600,cursor:"pointer"}},ic("schedule",showAttTimes?ACCENT:GRY,11),"Times")
        ),
        h("div",{style:{background:CARD,border:"1px solid "+BDR,borderRadius:16,padding:"14px",marginBottom:12}},
          h("div",{style:{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3,marginBottom:8}},
            ...
            (function(){
              ...
              for(var day=1;day<=days;day++){
                ...
                cells.push(h("div",{key:day,style:{aspectRatio:"1",borderRadius:6,background:s==="unmarked"?SFT:ATC[s]+"18",border:isTd?"2px solid "+ACCENT:"1px solid "+(s==="unmarked"?BDR:ATC[s]+"33"),display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:2}},
                  h("div",{style:{fontSize:11,fontWeight:isTd?800:500,color:s==="unmarked"?NVY:ATC[s]}},day),
                  showAttTimes&&ci&&ci.check_in_at?h("div",{style:{fontSize:5.5,color:GRY,fontWeight:700,lineHeight:1.2,textAlign:"center"}},
                    new Date(ci.check_in_at).toLocaleTimeString("en-IN",{hour:"numeric",minute:"2-digit"}).replace(" ","")
                  ):(!showAttTimes&&s!=="unmarked"?h("div",{style:{fontSize:6,color:ATC[s],fontWeight:700}},ATL[s]?ATL[s].slice(0,2):""):null)
                ));
              }
              return cells;
            })()
        ...
        // Summary
        myRecord?h("div",{style:{background:CARD,border:"1px solid "+BDR,borderRadius:16,padding:"14px"}},
          ...
          h("div",{style:{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}},
            (function(){
              ...
              return [["Present",p2,GRN],["Absent",a2,RED],["Half",h2,AMB],["Leave",l2,ACCENT]];
            })().map(function(item){
    ...
    function empTasks(){
      ...
      if(empSelTask){
        ...
        return h("div",{className:"fd"},
          ...
          h("div",{style:{background:CARD,borderRadius:16,padding:"14px",border:"1px solid "+BDR,marginBottom:8}},
              ...
              ["assigned","in_progress","completed","verified"].map(function(s,i){
                ...
                return [
                  h("div",{key:s,style:{display:"flex",flexDirection:"column",alignItems:"center",gap:2}},
                    h("div",{style:{width:22,height:22,borderRadius:"50%",background:isDone?(isCur?ACCENT:GRN):"#E2E8F0",display:"flex",alignItems:"center",justifyContent:"center"}},
                  ...
                  i<3?h("div",{key:s+"l",style:{flex:1,height:2,background:i<curIdx?GRN:"#E2E8F0",borderRadius:1,margin:"0 2px",marginBottom:14}}):null
                ];
              })
            ),
          ...
          return h("div",null,
            ...
            active.length>0?h("div",{style:{marginBottom:16}},
              ...
              active.map(function(t){
                ...
                return h("div",{key:t.id,onClick:function(){setEmpSelTask(t);},style:{background:CARD,borderRadius:14,padding:"12px 14px",marginBottom:8,border:"1px solid "+BDR,cursor:"pointer",borderLeft:"3px solid "+(t.status==="rejected"?RED:t.priority==="high"?RED:t.priority==="medium"?AMB:GRN)}},
              ...
              h("div",{style:{fontSize:11,fontWeight:700,color:GRY,letterSpacing:1,marginBottom:8}},"COMPLETED"),
                ...
                return h("div",{key:t.id,onClick:function(){setEmpSelTask(t);},style:{background:CARD,borderRadius:14,padding:"12px 14px",marginBottom:8,border:"1px solid "+BDR,cursor:"pointer",opacity:.7,borderLeft:"3px solid "+(t.status==="verified"?GRN:ACCENT)}},
    ...
    // ── Leave tab ──
    // ── Leave section — now lives as a card inside the Attendance tab, not its own nav tab.
    // Rewired to use the SAME state (leaveType/leaveFrom/leaveTo/leaveReason) and the SAME
    // applyLeave() function that the employer-facing leave flow already uses and that we already
    // fixed to persist to the real leave_requests table — this screen previously had its own
    // separate, never-fixed copy of the submission logic that only updated local state and never
    // actually reached the employer.
    function empLeaveCard(){
      ...
      return h("div",null,
        h("div",{style:{background:CARD,borderRadius:16,padding:"16px",marginBottom:14,border:"1px solid "+BDR}},
        ...
        showLeaveForm?h("div",{style:{background:CARD,borderRadius:16,padding:"16px",marginBottom:14,border:"1px solid "+BDR}},
          ...
          lbl("LEAVE TYPE"),
          chipSelect(leaveType,function(v){setLeaveType(v);},Object.keys(LEAVE_TYPES).map(function(lt){return {v:lt,l:LEAVE_TYPES[lt].name};}),{question:"Choose the leave type"}),
          h("div",{style:{display:"flex",gap:8,marginBottom:8}},
            h("div",{style:{flex:1}},lbl("FROM"),datePick(leaveFrom,function(v){setLeaveFrom(v);},{question:"Leave from"})),
            h("div",{style:{flex:1}},lbl("TO"),datePick(leaveTo,function(v){setLeaveTo(v);},{question:"Leave to"}))
          ),
          LEAVE_TYPES[leaveType]&&LEAVE_TYPES[leaveType].needsReason?h("div",null,
            lbl("REASON"),
            h("textarea",{value:leaveReason,onChange:function(e){setLeaveReason(e.target.value);},placeholder:"Enter reason...",style:{width:"100%",height:60,background:SFT,border:"1px solid "+BDR,borderRadius:10,padding:"10px 12px",fontSize:12,color:NVY,outline:"none",fontFamily:"inherit",resize:"none",marginBottom:8}})
          ):null,
          h("div",{style:{display:"flex",gap:8}},
            h("button",{onClick:applyLeave,disabled:leaveSubmitting,style:{flex:2,background:ACCENT,border:"none",borderRadius:10,padding:"12px",color:ACCENT_FG,fontSize:13,fontWeight:700,cursor:"pointer",opacity:leaveSubmitting?.6:1}},leaveSubmitting?"Sending...":"Submit"),
            h("button",{onClick:function(){setShowLeaveForm(false);},style:{flex:1,background:SFT,border:"1px solid "+BDR,borderRadius:10,padding:"12px",color:NVY,fontSize:12,cursor:"pointer"}},"Cancel")
          )
        ):h("button",{onClick:function(){setShowLeaveForm(true);},style:{width:"100%",background:ACCENT,border:"none",borderRadius:14,padding:"13px",color:ACCENT_FG,fontSize:13,fontWeight:700,cursor:"pointer",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"center",gap:8}},
        ...
        myLeaves.length>0?h("div",null,
          ...
          myLeaves.map(function(r){
            return h("div",{key:r.id,style:{background:CARD,borderRadius:14,padding:"12px 14px",marginBottom:8,border:"1px solid "+BDR,borderLeft:"3px solid "+(r.status==="approved"?GRN:r.status==="rejected"?RED:AMB)}},
      ...
      return h("div",{className:"fd"},
        ...
        h("div",{style:{background:NVY,borderRadius:18,padding:"16px",marginBottom:14,boxShadow:T.SHADOW_LG,border:"1px solid "+(themeMode==="light"?"rgba(255,255,255,.06)":"rgba(0,0,0,.08)")}},
          ...
          payData?(function(){
            ...
            var mutedC=themeMode==="light"?"rgba(255,255,255,.4)":"rgba(0,0,0,.45)";
            var sepC=themeMode==="light"?"rgba(255,255,255,.3)":"rgba(0,0,0,.3)";
            var addOns=(payMp.bonusTotal||0)+(payMp.otTotal||0);
            var terms=[
              {l:"GROSS",v:fmt(payData.gr),op:null,c:CARD},
              addOns>0?{l:"BONUS/OT",v:fmt(addOns),op:"+",c:posC}:null,
              {l:"DEDUCTIONS",v:totalDed>0?fmt(totalDed):"Nil",op:"\u2212",c:negC},
              payMp.loanDed>0?{l:"LOAN/EMI",v:fmt(payMp.loanDed),op:"\u2212",c:negC}:null,
              {l:"NET PAY",v:fmt(payMp.netFinal),op:"=",c:finalC}
            ].filter(Boolean);
            var tight=terms.length>4;
            var fs=terms.length>=6?10:terms.length>4?11.5:14;
            return [
              h("div",{key:"row",style:{display:"flex",alignItems:"flex-start",justifyContent:"center",gap:tight?4:8,marginBottom:14,flexWrap:"wrap",rowGap:10}},
                terms.map(function(t,i){
                  return h("div",{key:"t"+i,style:{display:"flex",alignItems:"center",gap:tight?4:8,flex:"0 1 auto"}},
                    t.op?h("div",{style:{color:sepC,fontSize:tight?14:18,fontWeight:300,flexShrink:0}},t.op):null,
                    h("div",{style:{textAlign:"center",minWidth:58}},
              ...
              h("div",{key:"hr",style:{height:1,background:themeMode==="light"?"rgba(255,255,255,.08)":"rgba(0,0,0,.10)",marginBottom:10}}),
              h("div",{key:"tot",style:{textAlign:"center"}},
                h("div",{style:{fontSize:10,color:CARD,opacity:.45,marginBottom:3}},"NET TAKE HOME"),
                h("div",{style:{fontSize:21,fontWeight:900,color:themeMode==="light"?"#4ADE80":"#059669",letterSpacing:-.5}},fmt(payMp.netFinal))
              )
            ];
          })():h("div",{style:{fontSize:12,color:"rgba(255,255,255,.5)",textAlign:"center",padding:"10px 0"}},"No salary data for this month")
        ),
        emp&&payData?h("div",null,
          // ── Simple breakdown ──
          h("div",{style:{background:CARD,borderRadius:16,padding:"14px",marginBottom:12,border:"1px solid "+BDR}},
            h("div",{style:{fontSize:11,fontWeight:700,color:GRY,letterSpacing:1,marginBottom:10}},"BREAKDOWN"),
            (emp.salaryType==="fixed"?[["Fixed Salary",fmt(payData.basic),NVY,1]]:
              [["Basic",fmt(payData.basic),NVY,1],["HRA",fmt(payData.hra),NVY,1],["Allowances",fmt(payData.allow),NVY,1]])
            .concat(payData.inc>0?[["Incentive",fmt(payData.inc),GRN,1]]:[])
            .concat(payMp.bonusTotal>0?[["Bonus",fmt(payMp.bonusTotal),GRN,1]]:[])
            .concat(payMp.otTotal>0?[["Overtime",fmt(payMp.otTotal),TEL,1]]:[])
            .concat(payData.pfE>0?[["PF Deduction",fmt(payData.pfE),RED,-1]]:[])
            .concat(payData.esiE>0?[["ESI Deduction",fmt(payData.esiE),RED,-1]]:[])
            .concat(payData.pt>0?[["Professional Tax",fmt(payData.pt),AMB,-1]]:[])
            .concat(payData.tds>0?[["TDS",fmt(payData.tds),RED,-1]]:[])
            .concat(payMp.loanDed>0?[["Loan/Advance EMI",fmt(payMp.loanDed),RED,-1]]:[])
            .map(function(item){
              return h("div",{key:item[0],style:{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid "+BDR}},
                ...
                h("span",{style:{fontSize:12,fontWeight:600,color:item[2]}},(item[3]<0?"\u2212":"")+item[1])
              );
            }),
    ...
    return h("div",{style:{display:"flex",flexDirection:"column",height:"100vh",background:T.BG,overflow:"hidden"}},
      ...
      h("div",{style:{background:CARD,padding:"12px 16px 10px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid "+BDR,flexShrink:0}},
        h("div",null,
          ...
          h("div",{style:{fontSize:17,fontWeight:800,color:NVY}},
            empDashTab==="home"?"Dashboard":empDashTab==="attendance"?"Attendance":empDashTab==="tasks"?"Work":"Pay"
          )
        ),
      ...
      h("div",{style:{display:"flex",background:CARD,borderTop:"0.5px solid "+BDR,padding:"6px 0 2px",flexShrink:0}},
        isTerminated?null:empNav("home","Dashboard","dashboard"),
        isTerminated?null:empNav("tasks","Work","task_alt"),
        empNav("attendance","Attendance","calendar_month"),
        empNav("profile","Pay","payments")
      ),
      renderNotifPanel(),
      renderInviteModal()
    );
      ...
      h("div",{style:{position:"absolute",top:56,right:8,width:320,maxHeight:"70vh",background:CARD,borderRadius:16,border:"1px solid "+BDR,overflow:"hidden",boxShadow:"0 8px 32px rgba(0,0,0,.15)"},onClick:function(e){e.stopPropagation();}},
        h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",borderBottom:"1px solid "+BDR}},
          ...
          h("div",{style:{display:"flex",gap:10}},
            myNotifs.some(function(n){return n.read;})?h("button",{onClick:deleteAllReadNotifs,style:{background:"none",border:"none",fontSize:11,color:RED,cursor:"pointer",fontWeight:600}},"Clear read"):null,
            myNotifs.some(function(n){return !n.read;})?h("button",{onClick:markAllNotifsRead,style:{background:"none",border:"none",fontSize:11,color:TEL,cursor:"pointer",fontWeight:600}},"Mark all read"):null
          )
        ),
        h("div",{style:{overflowY:"auto",maxHeight:"calc(70vh - 48px)"}},
          ...
          myNotifs.map(function(n){
            var iconMap={task_assigned:"assignment",task_completed:"task_alt",task_verified:"verified",task_rejected:"cancel",task_comment:"chat_bubble",leave_requested:"event_busy",leave_approved:"event_available",leave_rejected:"event_busy",kpi_scored:"insights",salary_revised:"payments",payslip_requested:"receipt_long"};
            var colorMap={task_assigned:"#4F46E5",task_completed:"#10B981",task_verified:"#10B981",task_rejected:"#EF4444",task_comment:"#4F46E5",leave_requested:"#F59E0B",leave_approved:"#10B981",leave_rejected:"#EF4444",kpi_scored:"#D97706",salary_revised:"#10B981",payslip_requested:"#D97706"};
            return h("div",{key:n.id,style:{display:"flex",gap:10,padding:"10px 14px",borderBottom:"1px solid "+BDR,background:n.read?"transparent":ACCENT+"08"}},
              h("div",{onClick:function(){
                markNotifRead(n.id);setShowNotifs(false);
                if(n.refType==="task")setTab("pro");
                if(n.refType==="leave"){
                  if(userRole==="employee"||userRole==="terminated_employee"){setEmpDashTab("attendance");}
                  else{
              ...
              h("div",{style:{display:"flex",flexDirection:"column",alignItems:"center",gap:6,flexShrink:0}},
                !n.read?h("div",{style:{width:8,height:8,borderRadius:"50%",background:ACCENT}}):null,
                h("button",{onClick:function(e){e.stopPropagation();deleteNotif(n.id);},title:"Delete",style:{background:"none",border:"none",padding:2,cursor:"pointer",display:"flex"}},ic("delete",GRY,14))
              )
            );
          })
        )
  ...
  function renderKpiCard(k){
    ...
    return h("div",{key:k.id,style:{background:CARD,border:"1px solid "+BDR,borderRadius:14,padding:"13px 14px",marginBottom:9,boxShadow:expanded?"0 2px 10px rgba(0,0,0,.06)":"none"}},
      h("div",{onClick:function(){setKpiExpandId(expanded?null:k.id);},style:{cursor:"pointer"}},
        h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}},
          h("div",{style:{flex:1,minWidth:0}},
            h("div",{style:{fontSize:13,fontWeight:700,color:NVY}},k.title||k.name),
            h("div",{style:{fontSize:10,color:GRY,marginTop:2,display:"flex",alignItems:"center",gap:4}},
              ic(k.assignType==="individual"?"person":k.assignType==="department"?"groups":"badge",GRY,11),
              k.assignType==="individual"?"Individual":k.assignType==="department"?"Dept: "+k.assignTarget:"Role: "+k.assignTarget
            )
          ),
          h("div",{style:{display:"flex",alignItems:"center",gap:6,flexShrink:0}},
            h("div",{style:{fontSize:9,fontWeight:800,padding:"4px 10px",borderRadius:20,background:meta.bg,color:meta.color,letterSpacing:.3}},meta.label.toUpperCase()),
            ic(expanded?"expand_less":"expand_more",GRY,17)
          )
        ),
        h("div",{style:{display:"flex",alignItems:"baseline",justifyContent:"space-between",marginBottom:6}},
          h("div",{style:{fontSize:11,color:GRY}},h("span",{style:{fontSize:16,fontWeight:800,color:NVY}},prog)," / "+target+" "+k.unit),
          h("div",{style:{fontSize:15,fontWeight:800,color:meta.color}},pct+"%")
        ),
        h("div",{style:{height:8,background:SFT,borderRadius:4,overflow:"hidden"}},
          h("div",{style:{height:"100%",background:"linear-gradient(90deg,"+meta.color+"cc,"+meta.color+")",borderRadius:4,width:pct+"%",transition:"width .4s ease"}})
        )
      ),
      expanded?h("div",{style:{borderTop:"1px solid "+BDR,marginTop:12,paddingTop:12}},
        h("div",{style:{display:"flex",gap:8,marginBottom:10}},
          h("div",{style:{flex:1,background:SFT,borderRadius:10,padding:"8px 10px"}},h("div",{style:{fontSize:8.5,color:GRY,fontWeight:700,letterSpacing:.3}},"START"),h("div",{style:{fontSize:11.5,color:NVY,fontWeight:700,marginTop:2}},k.startDate||"-")),
          h("div",{style:{flex:1,background:SFT,borderRadius:10,padding:"8px 10px"}},h("div",{style:{fontSize:8.5,color:GRY,fontWeight:700,letterSpacing:.3}},"DUE"),h("div",{style:{fontSize:11.5,color:NVY,fontWeight:700,marginTop:2}},k.dueDate||"-")),
          h("div",{style:{flex:1,background:SFT,borderRadius:10,padding:"8px 10px"}},h("div",{style:{fontSize:8.5,color:GRY,fontWeight:700,letterSpacing:.3}},"UNIT"),h("div",{style:{fontSize:11.5,color:NVY,fontWeight:700,marginTop:2}},k.unit))
        ),
        k.followUpRemarks?h("div",{style:{background:ACCENT+"0D",border:"1px solid "+ACCENT+"22",borderRadius:10,padding:"9px 11px",marginBottom:10,fontSize:11.5,color:NVY,lineHeight:1.4}},h("b",{style:{color:ACCENT}},"Latest remark: "),k.followUpRemarks):null,
        // ── Update progress ──
        updating?h("div",{style:{background:TEL+"0D",border:"1px solid "+TEL+"35",borderRadius:12,padding:12,marginBottom:10}},
          h("div",{style:{fontSize:10,fontWeight:700,color:TEL,marginBottom:8,letterSpacing:.3}},"LOG PROGRESS UPDATE"),
          h("input",{type:"number",value:kpiProgressInput,onChange:function(ev){setKpiProgressInput(ev.target.value);},placeholder:"Current progress ("+k.unit+")",autoFocus:true,style:{width:"100%",background:CARD,border:"1px solid "+BDR,borderRadius:9,padding:"9px 10px",fontSize:12.5,color:NVY,outline:"none",fontFamily:"inherit",marginBottom:7,boxSizing:"border-box"}}),
          h("input",{type:"text",value:kpiRemarkInput,onChange:function(ev){setKpiRemarkInput(ev.target.value);},placeholder:"Follow-up remark (optional)",style:{width:"100%",background:CARD,border:"1px solid "+BDR,borderRadius:9,padding:"9px 10px",fontSize:12.5,color:NVY,outline:"none",fontFamily:"inherit",marginBottom:8,boxSizing:"border-box"}}),
          h("div",{style:{display:"flex",gap:7}},
            h("button",{onClick:function(){
            ...
            },style:{flex:2,background:NVY,border:"none",borderRadius:9,padding:"9px",fontSize:12,fontWeight:700,color:CARD,cursor:"pointer"}},"Save Update"),
            h("button",{onClick:function(){setKpiUpdateOpenId(null);},style:{flex:1,background:SFT,border:"1px solid "+BDR,borderRadius:9,padding:"9px",fontSize:12,color:GRY,cursor:"pointer"}},"Cancel")
          )
        ):h("button",{onClick:function(){setKpiUpdateOpenId(k.id);setKpiProgressInput(String(prog));setKpiRemarkInput("");},style:{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:6,background:TEL+"12",border:"1.5px solid "+TEL+"30",borderRadius:11,padding:"10px",fontSize:12,fontWeight:700,color:TEL,cursor:"pointer",marginBottom:10}},ic("edit",TEL,14),"Update Progress"),
        // ── Review/update history ──
        history.length>0?h("div",{style:{marginBottom:10}},
          h("div",{style:{fontSize:9,fontWeight:700,color:GRY,letterSpacing:.3,marginBottom:6}},"UPDATE HISTORY"),
          history.map(function(u){
            return h("div",{key:u.id,style:{display:"flex",gap:8,alignItems:"flex-start",padding:"6px 0",borderBottom:"1px solid "+BDR}},
              h("div",{style:{width:6,height:6,borderRadius:"50%",background:ACCENT,flexShrink:0,marginTop:5}}),
              h("div",{style:{flex:1}},
                h("div",{style:{fontSize:11,fontWeight:600,color:NVY}},u.progress+" "+k.unit+(u.remark?" — "+u.remark:"")),
                h("div",{style:{fontSize:9.5,color:GRY,marginTop:1}},new Date(u.createdAt).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"numeric"})+" "+new Date(u.createdAt).toLocaleTimeString("en-IN",{hour:"numeric",minute:"2-digit"}))
              )
            );
          })
      ...
      h("div",{style:{background:CARD,padding:"14px 16px 12px",borderBottom:"1px solid "+BDR,position:"sticky",top:0,zIndex:50}},
        h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center"}},
          ...
          h("div",{style:{display:"flex",alignItems:"center",gap:8,position:"relative"}},
            ...
            h("button",{onClick:function(){setShowNotifs(true);},title:"Notifications",style:{width:38,height:38,borderRadius:11,background:SFT,border:"1px solid "+BDR,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",position:"relative"}},
              ic("notifications",unreadNotifs>0?ACCENT:NVY,19),
              unreadNotifs>0?h("div",{style:{position:"absolute",top:2,right:2,width:15,height:15,borderRadius:"50%",background:RED,display:"flex",alignItems:"center",justifyContent:"center",border:"1.5px solid "+CARD}},
                h("div",{style:{fontSize:8,fontWeight:700,color:"#fff"}},unreadNotifs>9?"9+":String(unreadNotifs))
              ):null
            ),
            ...
            prof?h("div",{style:{position:"absolute",top:42,right:0,background:CARD,border:"1px solid "+BDR,borderRadius:13,padding:6,minWidth:185,boxShadow:T.SHADOW_LG,zIndex:200}},
              ...
              h("button",{onClick:function(){setTab("settings");setSettTab("account");setProf(false);},style:{width:"100%",background:"none",border:"none",borderRadius:7,padding:"7px 11px",textAlign:"left",fontSize:12,fontWeight:600,color:NVY,cursor:"pointer",display:"flex",alignItems:"center",gap:6}},ic("settings",NVY,14),"Settings"),
